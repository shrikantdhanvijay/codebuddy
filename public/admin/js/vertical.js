var vertical = {
    results_table: "vertical_list_table",
    ...
    init: function () {
        ...
        $("#save_form").click(function (e) {
            if ($("#saveVertical_form").valid()) {
                vertical.save();
            }
        });

        //        $('#findSearch').click(function(){
        //           var oTable = $('#'+vertical.results_table).DataTable();
        //           var fromData = $('#'+vertical.searchForm).serialize();
        //           oTable.search(fromData).draw();
        //        });
        //
        //        // Custom search reset
        //        $('#searchreset').click(function () {
        //            vertical.clearFormNew(vertical.searchForm);
        //        });

        // Delete Record
        $(document).on("click", ".deleteRecord", function () {
            var id = $(this).attr("id");
            vertical.deleteRecord(id);
        });

        // Validation Start
        $("#saveVertical_form").validate({
            errorElement: "div",
    ...
    verticalList: function () {
        var post_url = SITE_URL + "admin/master/vertical_list";
        $.ajaxSetup({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            },
        });
        $("#" + vertical.results_table).DataTable({
            processing: true,
            serverSide: true,
            searching: true,
            responsive: false,
        ...
        //        $("#vertical_list_table_filter").html(inputHtml)
        $("#vertical_list_table_wrapper").css("padding", "0px");
    ...
    save: function () {
        var post_url = SITE_URL + "admin/master/vertical_store";
        // Get form
        var form = $("#saveVertical_form")[0];
        ...
        $.ajax({
            ...
            success: function (rs) {
                ...
                if (rs.status == 1) {
                    ...
                    setInterval(function () {
                        window.location = SITE_URL + "admin/master/verticals/";
            ...
            error: function (rs) {
                ...
                $(".custom_validation_message").hide();
                // $('#saveVertical_form').addClass('was-validated');
    ...
    deleteRecord: function (id) {
        if (confirm(comman.delete_confirm)) {
            $.ajax({
                type: "post",
                url: SITE_URL + "admin/vertical/delete",
                headers: {
                ...
                success: function (rs) {
                    ...
                    if (status == 1) {
                        ...
                        setInterval(function () {
                            window.location =
                                SITE_URL + "admin/master/verticals";
                        }, 1000);
                    } else {
                        comman.toast("success", rs.message);
                    }
