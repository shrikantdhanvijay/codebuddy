var sub_company = {
    results_table: "sub_company_list_table",
    ...
    init: function () {
        ...
        $("#save_form").click(function (e) {
            if ($("#savesub_company_form").valid()) {
            sub_company.save();
            }
        });

        //        $('#findSearch').click(function(){
        //           var oTable = $('#'+sub_company.results_table).DataTable();
        //           var fromData = $('#'+sub_company.searchForm).serialize();
        //           oTable.search(fromData).draw();
        //        });
        //
        //        // Custom search reset
        //        $('#searchreset').click(function () {
        //            sub_company.clearFormNew(sub_company.searchForm);
        //        });

        // Delete Record
        $(document).on("click", ".deleteRecord", function () {
            var id = $(this).attr("id");
            sub_company.deleteRecord(id);
        });

        // // Validation Start
              $("#savesub_company_form").validate({
                  errorElement: "div",
    ...
    sub_companyList: function () {
        var post_url = SITE_URL + "admin/master/sub_company_list";
        $.ajaxSetup({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            },
        });
        $("#" + sub_company.results_table).DataTable({
            processing: true,
            serverSide: true,
            searching: true,
            responsive: false,
        ...
        //        $("#sub_company_list_table_filter").html(inputHtml)
        $("#sub_company_list_table_wrapper").css("padding", "0px");
    ...
    save: function () {
        var post_url = SITE_URL + "admin/master/sub_company_store";
        // Get form
        var form = $("#savesub_company_form")[0];
            ...
            success: function (rs) {
                ...
                if (rs.status == 1) {
                    ...
                    setInterval(function () {
                        window.location = SITE_URL + "admin/master/sub_company";
    ...
    deleteRecord: function (id) {
        if (confirm(comman.delete_confirm)) {
            $.ajax({
                type: "post",
                url: SITE_URL + "admin/sub_company/delete",
                headers: {
                ...
                success: function (rs) {
                    ...
                    if (status == 1) {
                        ...
                        setInterval(function () {
                            window.location =
                                SITE_URL + "admin/master/sub_company";
                        }, 1000);
                    } else {
                        comman.toast("success", rs.message);
                    }
