var cost_type = {
    results_table: 'cost_type_list_table',
...
//        });
        ...
        $('#save_form').click(function (e) {
              if ($("#savecost_type_form").valid()) {
            cost_type.save();
        }
        });

//        $('#findSearch').click(function(){
//           var oTable = $('#'+cost_type.results_table).DataTable();
//           var fromData = $('#'+cost_type.searchForm).serialize();
//           oTable.search(fromData).draw();
//        });
//
//        // Custom search reset
//        $('#searchreset').click(function () {
//            cost_type.clearFormNew(cost_type.searchForm);
//        });

        // Delete Record
        $(document).on('click', '.deleteRecord', function () {
            var id = $(this).attr('id');
            cost_type.deleteRecord(id);
        });

  // // Validation Start
        $("#savecost_type_form").validate({
            errorElement: "div",
    ...
    clearFormNew: function (formId) {
        $('#' + cost_type.searchForm).find('input, select, textarea').val('').trigger("change");
        var oTable = $('#' + cost_type.results_table).DataTable();
        var fromData = $('#' + cost_type.searchForm).serialize();
        oTable.search('formSearch:' + fromData).draw();
    },
    cost_typeList: function () {
        var post_url = SITE_URL + 'admin/master/cost_type_list';
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            }
        });
        $('#' + cost_type.results_table).DataTable({
            processing: true,
            serverSide: true,
            searching: true,
            responsive: false,
...
//        $("#cost_type_list_table_filter").html(inputHtml)
        $("#cost_type_list_table_wrapper").css("padding", "0px");
    ...
    save: function () {
        var post_url = SITE_URL + 'admin/master/cost_type_store';
        // Get form
        var form = $('#savecost_type_form')[0];
...
//                    $('#saveAmenity_form')[0].reset();
                    setInterval(function () {
                        window.location = SITE_URL + 'admin/master/cost_type';
...
//                    comman.toast('error', comman.session_expired);
    ...
    deleteRecord: function (id) {
        if (confirm(comman.delete_confirm)) {
            $.ajax({
                type: "post",
                url: SITE_URL + 'admin/cost_type/delete',
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')},
                data: {id: id},
                dataType: 'json',
                success: function (rs) {
